...
// Gets x for a pol
template <class T>
std::complex<double> newton(polynomial<T> pol) {
  if (std::complex<double>(pol[pol.degree()]) == std::complex<double>(0)) return std::complex<double>(0);
  polynomial<T> deriv = pol.derivative();

  std::complex<double> x = {randomFloat(-1, 1), randomFloat(-1, 1)};
  while (std::abs(deriv.eval(x) - std::complex<double>(0)) < 0.0000001) {
    x = {randomFloat(-1, 1), randomFloat(-1, 1)};
  }
  T r = pol.eval(x);
  T d = deriv.eval(x);

  int i = 0;
  while (std::abs(r) > 0.0000001 && i < 1000) {
    T delta = r / d;
    x -= delta;
    r = pol.eval(x);
    d = deriv.eval(x);
...
template <class T>
std::vector<std::complex<double>> solve(polynomial<T> pol) {
  polynomial<T> p = pol;
  int degree = p.degree();
  ...
  while (degree != 0) {
    ...
    polynomial<T> q = polynomial<T>({1, -root});
    p /= q;
    degree = p.degree();
  }
